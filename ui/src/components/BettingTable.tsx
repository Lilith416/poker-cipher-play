...
import { LockKeyhole, Loader2, Timer, Users, Crown, Shield, BookmarkCheck, PlugZap, Filter } from "lucide-react";
...
const BettingTable = () => {
      ...
      const records = await Promise.all(
        ids.map(async (id) => {
          try {
            ...
            // Parse game summary from raw contract response
            const summary = parseGameSummary(summaryRaw);
            ...
            const participant = parseParticipant(participantRaw as readonly unknown[]);
  ...
  const handleCreateGame = async () => {
    try {
      ...
      toast({
        title: "Create failed",
        description: `Failed to create game: ${errorMessage}`,
        variant: "destructive",
      });
    }
  };
  ...
  const handleDebugReveal = async (gameId: bigint, secretNumber?: number) => {
    try {
      ...
      if (actualSecretNumber === undefined) {
        const storedData = findStoredGameData();
        if (storedData === null) {
          toast({
        ...
        actualSecretNumber = storedData.secretNumber;
      }
