...
/// @dev This contract uses fully homomorphic encryption to maintain privacy
/// of both the secret number and player predictions throughout the game lifecycle.
contract EncryptedHighLow is SepoliaConfig, ReentrancyGuard {
    ...
    function joinGame(
        ...
        bytes calldata inputProof
    ) external payable gameExists(gameId) notEmergencyStopped {
        Game storage g = _games[gameId];
    ...
    function fairReveal(
        ...
        require(originalSecretNumber >= 1 && originalSecretNumber <= 10, "EncryptedHighLow: invalid secret number range");
