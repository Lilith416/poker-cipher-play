...
describe("EncryptedHighLow", () => {
  before(function () {
    if (!fhevm.isMock) {
      ...
      // Skip tests on live networks where FHE operations are non-deterministic.
      this.skip();
    }
  });
  ...
  it("stores the encrypted secret and allows decryption through fhevm helper", async () => {
    ...
    const secretNumberHash = ethers.keccak256(ethers.solidityPacked(["uint8", "bytes32"], [7, salt]));
    await contract
      .connect(creator)
      .createGame(encryptedSecret.handles[0], encryptedSecret.inputProof, secretNumberHash, MIN_STAKE, BigInt(now + 3600), ONE_ETHER / 10n, true, {
        value: ONE_ETHER / 10n,
      });
  ...
  it("runs a full round lifecycle with encrypted guesses and payouts", async () => {
    ...
    const salt = ethers.randomBytes(32);
    const secretNumberHash = ethers.keccak256(ethers.solidityPacked(["uint8", "bytes32"], [8, salt]));

    await contract
      .connect(creator)
      .createGame(secret.handles[0], secret.inputProof, secretNumberHash, MIN_STAKE, endTime, ONE_ETHER / 10n, true, {
        value: ONE_ETHER / 10n,
      });
